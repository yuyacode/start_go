...
func map_method() {
	...
	// make関数を使ってマップを生成する際に、２番目の引数に「予想されるマップの要素数に基づいて、初期で割り当てるメモリのサイズ」をヒントとしてGoランタイムに提供することが可能
	// 整数で指定可能
	// スライスにおける容量とは意味合いが異なる
	// マップに格納される要素数をもとに、Goランタイムが最適なメモリ領域を確保するための一種の「ヒント」として機能する
	// これにより、マップに要素を追加する際のメモリの再割り当ての回数を減らすことができ、大量の要素を扱う際のパフォーマンス向上を期待できる
	// 指定された整数の要素を格納するのに十分なメモリスペースを確保するようにGoランタイムに指示する
	// この数値は厳密な容量を意味するものではなく、あくまで最適化のためのヒントとして機能する
	// 指定した整数以上の要素を追加することは可能であり、その場合は必要に応じてメモリの再割り当てが行われる
	// 要素数の少ないマップに細かく指定するのは、煩雑なだけで意味をなさない
	// 要素数が膨大な巨大なマップを構築する場合であれば、パフォーマンスの向上を期待できるかもしれない
	// 要素の追加や削除が頻繁に行われる大規模なマップを扱う場合には、適切な初期サイズの指定が有効な最適化手段となることがある
	map_with_space := make(map[int]string, 500)
