...
func slice() {
	...
	// スライスを使って可変長引数に値を渡すことが可能
	// スライスに...を付加することで、スライスの要素１つ１つを展開して、可変長引数に渡している
	// s := []int {1, 2, 3, 100, 100}
	// fmt.Println(sum(s...))
...
// s ...int という引数定義は、任意個数の引数を[]int型のスライスとして受け取る
// 可変長引数とは、関数が任意の数の引数を受け取ることができる機能や仕組みのこと
// 可変長引数とは、任意の個数の引数自体を指している訳ではない
// func sum(s ...int) int {
// 	n := 0
// 	for _, v := range s {
// 		n += v
